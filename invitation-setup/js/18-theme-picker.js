...
// → làm nút trông như đã khoá. Gọi sau khi nạp dữ liệu, lúc IS_THEME_LOCKED đã có
// giá trị thật.
//
...
async function openThemePicker() {
  // Thiệp đã thanh toán: không mở bảng chọn nữa, bung lời giải thích ngay dưới nút.
  // Chặn ở ĐÂY chứ không chỉ ở _applyThemeChange để khách khỏi phải lướt hết danh
  // sách mẫu rồi mới biết là không đổi được.
  if (IS_THEME_LOCKED) {
    document
      .getElementById("theme-lock-pop")
      ?.toggle(document.getElementById("header-theme-btn"));
    return;
  }
