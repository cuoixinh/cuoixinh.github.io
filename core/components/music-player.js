...
(function () {
  ...
  //
  // BẢNG MÀU: nền thanh + khối tóm tắt là --surface-tint-rgb (kem sáng), nên mọi
  // thứ vẽ trên đó — chữ, icon, nút, vạch ngăn, dải tiến trình — lấy
  // --text-body-rgb với độ mờ khác nhau. Đừng quay lại màu trắng: trên nền kem nó
  // biến mất. Người dùng chọn màu riêng ở panel "Thành phần" thì .cx-tint-* trong
  // styles/_music-player.css đè lên (đặc hiệu cao hơn nên luôn thắng).
  const barRow = `
    <div
  ...
  // Mẫu gọn "nút tròn": class .cx-mw-* (styles/_music-player.css), kích thước
  // bên trong bằng `em` → kéo to nhỏ là cả widget phóng theo.
  const SVG = {
  ...
  function build(opts) {
    ...
    if (variant === "mini") {
      // Mẫu nút tròn không có bản fixed-top (thanh ngang): nó sinh ra để thả lên
      // thiệp, hoặc neo góc màn qua chrome "fixed-corner".
      node.className = "cx-mw cx-mw-mini";
      node.innerHTML = miniBody;
