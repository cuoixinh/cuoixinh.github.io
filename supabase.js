...
async function fetchWedding() {
    ...
    const res = await fetch(
        `${SUPABASE_URL}/rest/v1/weddings?slug=eq.${slug}&select=*`,
        {
            headers: {
...
function toLunar(dateStr) {
    ...
    const result = convertSolarToLunar(date.getDate(), date.getMonth() + 1, date.getFullYear());
...
function renderWedding(w) {
    ...
    setText('invite-month-year', `Tháng ${ceremonyDateObj.getMonth() + 1} · ${ceremonyDateObj.getFullYear()}`);
    ...
    setText('party-datetime', `${w.groom_party_time} - ${partyDate.getDate()}.${String(partyDate.getMonth()+1).padStart(2,'0')}.${partyDate.getFullYear()}`);
